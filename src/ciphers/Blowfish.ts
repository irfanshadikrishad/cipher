...
export class Blowfish extends Cipher {
  ...
  private F(x: number): number {
    ...
    const d = x & 0xff
    return toUint32(
      toUint32(toUint32(this.S[0][a] + this.S[1][b]) ^ this.S[2][c]) +
        this.S[3][d]
    )
  }
  ...
  private processBytes(input: Uint8Array, mode: 'enc' | 'dec'): Uint8Array {
    const padded =
      input.length % 8 === 0
        ? input
        : (() => {
    ...
    for (let i = 0; i < padded.length; i += 8) {
      ...
      const [nl, nr] =
        mode === 'enc' ? this.encryptBlock(l, r) : this.decryptBlock(l, r)
